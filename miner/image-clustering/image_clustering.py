...
class image_clustering:
	...
	def __init__(self, folder_path="/mnt/d/scrapings/yields_sample", n_clusters=10, max_examples=None, use_imagenets=False, use_pca=False):
		paths = os.listdir(folder_path)
		print("!!!!!!!! Meme Source:"+folder_path+"!!!!!!!!!")
		...
		del paths
		try: #delete the output folder
			shutil.rmtree("output")
			print("output folder deleted")
		except FileExistsError: #
			pass
		except FileNotFoundError:
			pass #no output folder to delete
		print("\n output folders created.")
		os.makedirs("output")
		for i in range(self.n_clusters):
			os.makedirs("/mnt/d/"+"output/" + str(self.max_examples) + "_cluster" + str(i))
	...
	def clustering(self):
		...
		for i in range(self.max_examples):
			src = self.folder_path+"/"+self.image_paths[i]
			dst="/mnt/d/output/" + str(self.max_examples) + "_cluster"+str(predictions[i])+"/sample_"+str(i)+"."+src[-3:]
			print("Destination: " + dst + "\nSource: " + src)
			dest_path = shutil.copy2(src,dst)
			print(dest_path)
...
if __name__ == "__main__":
	...
	data_path = "/mnt/g/Scraping Yields/instagram/memes/" # path of the folder that contains the images to be considered for the clustering (The folder must contain only image files)
	#data_path ="/mnt/d/scrapings/yields_sample" 
	max_examples = 5000 # number of examples to use, if "None" all of the images will be taken into consideration for the clustering
	...
	#no data path
	#temp = image_clustering(number_of_clusters, max_examples, use_imagenets, use_pca)
	temp.load_images()
