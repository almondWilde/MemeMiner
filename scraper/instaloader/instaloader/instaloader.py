...
class Instaloader:
    ...
    def download_post(self, post: Post, target: Union[str, Path]) -> bool:
        ...
        ##record tags from captions
        try:
            with open(filename+'.txt', 'r') as caption:
                for line in caption.readlines():
                    for word in line.split(' '):
                        if word.startswith('#') and (word.endswith('meme') or word.endswith('joke') or word.endswith('memes') or word.endswith('jokes')):
                             with open('meme-tags.txt', 'a') as meme_list:
                                word = word.strip()
                                meme_list.write('\n'+word[1:])
